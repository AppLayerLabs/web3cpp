...
bool KeyManager::recode(Address const& _address, std::string const& _newPass, std::string const& _hint, std::function<std::string()> const& _pass, KDF _kdf)
{
			...
			for (auto const& i: s[1])
			{
				...
				if (uuid)
				{
					if (m_store.contains(uuid))
					{
						m_addrLookup[addr] = uuid;
						m_uuidLookup[uuid] = addr;
						m_keyInfo[addr] = KeyInfo(h256(i[2]), std::string(i[3]), i.itemCount() > 4 ? std::string(i[4]) : "");
...
Secret KeyManager::secret(Address const& _address, std::function<std::string()> const& _pass, bool _usePasswordCache) const
{
	return secret(m_addrLookup.at(_address), _pass, _usePasswordCache);
}

Secret KeyManager::secret(h128 const& _uuid, std::function<std::string()> const& _pass, bool _usePasswordCache) const
{
...
std::string KeyManager::getPassword(h128 const& _uuid, std::function<std::string()> const& _pass) const
{
	h256 ph;
...
std::string KeyManager::getPassword(h256 const& _passHash, std::function<std::string()> const& _pass) const
{
...
h128 KeyManager::import(Secret const& _s, std::string const& _accountName, std::string const& _pass, std::string const& _passwordHint)
{
...
void KeyManager::importExisting(h128 const& _uuid, std::string const& _info, std::string const& _pass, std::string const& _passwordHint)
{
...
void KeyManager::importExisting(h128 const& _uuid, std::string const& _accountName, Address const& _address, h256 const& _passHash, std::string const& _passwordHint)
{
