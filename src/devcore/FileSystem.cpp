...
fs::path dev::getDataDir(std::string _prefix)
{
	if (_prefix.empty())
		_prefix = "ethereum";
...
fs::path dev::getDefaultDataDir(std::string _prefix)
{
	if (_prefix.empty())
		_prefix = "ethereum";
...
fs::path dev::appendToFilename(fs::path const& _orig, std::string const& _suffix)
{
