...
void Tests::testDatabaseCRUD() {
  ...
  for (std::pair<std::string, std::string> testPair : testData) {
    ...
    logC += "* {'" + testPair.first + "', '" + testPair.second
      + "'} added: " + ((added) ? "YES" : "NO") + "\n";
    ctC += added;
  }
  ...
  if (okC) {
    ...
    for (std::pair<std::string, std::string> testPair : testData) {
      ...
      if (!isSameValue) allSameValue = false;
    }
    ...
    logR += std::string("* getAllKeys() matches test keys: ")
      + ((allKeyCheck) ? "YES" : "NO") + "\n";
    logR += std::string("* getAllValues() matches test values: ")
      + ((allValueCheck) ? "YES" : "NO") + "\n";
    logR += std::string("* getAllPairs() matches test pairs: ")
      + ((allPairCheck) ? "YES" : "NO") + "\n";
  ...
  // Results
  if (!okC || !okR || !okU || !okD) {
    failed("One or more subtests failed");
  } else {
    passed();
  }

  db.dropDatabase(); // Cleanup every time
  std::cout << ((curPass) ? "PASSED" : "FAILED") << std::endl;
  logStream << "* Test: " << __func__ << std::endl
    << "* Test data: " << std::endl;
    for (std::pair<std::string, std::string> testPair : testData) {
      logStream << "* " << testPair.first << " - " << testPair.second << std::endl;
    }
    logStream << logC << logR << logU << logD << "* Test result: " <<
